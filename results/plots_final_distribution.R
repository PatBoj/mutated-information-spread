...
data <- getData("30_09_2021_hist/")
data <- rbind(data, getData("30_09_2021_others_hist/"))
data <- data[data$sim == 0, -9]
data <- data[data$tau == "-0.4" | data$tau == "0.2" | data$tau == "0.8", ]
data$tau <- droplevels(data$tau)
data$eta <- as.factor(data$eta, levels = c("0.2", "0.0"))
...
data[data$eta == "0.00", 12] <- data[data$eta == "0.0", 12] + 0.075

myLabels <- list("$\\alpha = 0.2$", "$\\alpha = 0.00$")
...
popularityHistogram <- function(histogram) {
  plot <- ggplot(data = histogram) + 
    ...
    #         width = 0.09) +
    geom_errorbar(aes(x = bin_x, ymin = (bin_y - lengthSD)/2, ymax = (bin_y + lengthSD)/2, y = bin_y/2, color = factor(eta, levels = rev(levels(eta)))),
                  stat = "identity", 
...
# 5.44
tikz(file = "plots/Results_plot_ER-no-competition.tex", width = inch(15), height = inch(6))
popularityHistogram(data[data$network_type == "ER" & data$type == "without_competition", ])
dev.off()
