...
getData <- function(dirName, type) {
  ...
  data$type <- cbind(rep(type, nrow(data)))
  data$type <- as.factor(data$type)
  return(data)
}

data <- getData("26_06_custom_fb_hist/", "1")
data <- rbind(data, getData("26_06_custom_FR_hist/", "2"))
data <- rbind(data, getData("26_06_custom_DE_hist/", "3"))
data <- data[, -9]
...
labels_abc <- data.frame(type = c("1", "1", "1", "2", "2", "2", "3", "3", "3"),
                         tau = c(-0.4, 0.2, 0.8, -0.4, 0.2, 0.8, -0.4, 0.2, 0.8),
...
popularityHistogram <- function(histogram) {
  plot <- ggplot(data = histogram) + 
    ...
    scale_x_continuous(limits = c(-.05, 4),
                       breaks = seq(0, 5),
...
# 5.44
tikz(file = "plots/Results_plot_real-topology-a.tex", width = inch(17), height = inch(17))
popularityHistogram(data)
dev.off()
...
#      panel.background=element_blank(),
#      legend.key=element_rect(fill = NA, color = NA),
#      legend.background=element_rect(fill = (eta("white", 0))),
#      legend.title=element_blank(),
#      legend.position="top",
#      legend.box.background=element_rect(colour = "black"),
#      legend.spacing.y = unit(0, "mm"),
