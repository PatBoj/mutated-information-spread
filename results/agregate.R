...
for(i in 1:length(filesGroups)) {
  ...
  length <- sapply(seq_along(tempHist$breaks[-1]), function(j) {
    if(j == 1) {
      condition <- log10(rawData$repetition) >= tempHist$breaks[j] & log10(rawData$repetition) <= tempHist$breaks[j+1]
    } else {
      condition <- log10(rawData$repetition) > tempHist$breaks[j] & log10(rawData$repetition) <= tempHist$breaks[j+1]
    }
    ...
    if(sum(condition) == 0) {
      ...
      avg <- sum(tempData$repetition * tempData$avg_length) / sum(tempData$repetition)
      sd <- sqrt(sum(tempData$repetition * (tempData$avg_length - avg)**2) / sum(tempData$repetition))
      c(avg, sd)
    }
  })
